...
void MainMenu::update() {
    ...
    DrawTextEx(c_font, c_titleText, Vector2{ .x = 500, .y = 100 }, c_buttonFontSize, c_fontSpacing, c_defaultTextColor);

    DrawTextEx(c_font, c_startButtonText, c_startButtonPos, c_buttonFontSize, c_fontSpacing, m_currentStartButtonTextColor);
    if (isMouseCollidingWithStartButton()) {
        m_currentStartButtonTextColor = c_startButtonTextColorHighlight;
        if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
            DrawRectangle(0, 0, 800, 600, BLACK);
            m_game->setGameState(GameState::GAMEPLAY);
        }
    } else {
        m_currentStartButtonTextColor = c_defaultTextColor;
    }
    DrawTextEx(c_font, c_exitButtonText, c_exitButtonPos, c_buttonFontSize, c_fontSpacing, m_currentExitButtonTextColor);
    if (isMouseCollidingWithExitButton()) {
        m_currentExitButtonTextColor = c_exitButtonTextColorHighlight;
        if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
            m_game->setGameState(GameState::EXITING);
        }
    } else {
        m_currentExitButtonTextColor = c_defaultTextColor;
    }
}
